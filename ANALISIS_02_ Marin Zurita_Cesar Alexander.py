...
def flujo(fila):
    """
    multiplico la cantidad trasportada por la frecuencia de flujo 
    para obtener el flujo de trasporte que pasa por la ruta
    para mejorar la tabla se divide entre 10 mil el flujo 

    """  
    resultado=(fila['cantidad trasportada']*fila['frecuencia de uso'])/10000
    return resultado
...
def porcentaje(fila,total=países['total_value'].sum()):
    ...
    la cantidad trasportada / el total de cantidad trasportada hasta el momento 
    por 100
    """
...
def menu1():
    ...
    la cantidad de trasporte de cada ruta se obtiene al usar la funcion sum()
    ...
    rutas=tabla_date.groupby(["origin","destination",'transport_mode']).agg({
        "total_value":'sum','company_name':'count'}).rename(columns={
        'total_value':'cantidad trasportada','company_name':'frecuencia de uso'})
...
def menu2():
    ...
    la cantidad de trasporte que pasa por ese medio 
    ...
    medio=tabla_date.groupby(['year','transport_mode']).agg({"total_value":'sum'}).rename(
        columns={'total_value':'Cantidad de trasporte'}).reset_index(level=['year','transport_mode'])
    """
    creo un datafreme que estara ordenado por la cantidad de trasporte del mayor al menor
    y renombro year y transport_mode
    """
    medio_grafica=medio.sort_values('Cantidad de trasporte',ascending=0).rename(
    ...
    sns.relplot(x="Año", y="Cantidad de trasporte", hue="Modo de trasporte", 
        kind="line",data=medio_grafica)

def menu3():
    #Valor total de importaciones y exportaciones
    """
    haciendo uso de las variables fijas se crea un dataframe países por la cantidad de trasporte
    con una columna donde se generara el porcentaje acumulado usando la funcion porcentaje y cumsum() 
    """
    ...
    y renombro total_value por cantidad trasportada
    """
    acumulado=países_orden.sort_values('porcentaje_acumulado').rename(columns={
        'total_value':'Cantidad trasportada'})
    ...
    paises_grafica.plot(kind='pie',y='Cantidad trasportada', subplots=True, 
